...
void ADC_compare(unsigned int compare_value);
...
ADC->ADCR &= ~(0x01ul << 31); // A/D result is unsigned format
...
ADC_compare(COMPARE_VALUE);
}
//ADC interrrupt compare configuration
void ADC_compare(unsigned int compare_value){
ADC->ADCMPR[0] |= (0x07ul << 3); // 111:channel 7 selected
ADC->ADCMPR[0] |= (compare_value << 16); // comparison data is 2457 or 3V
ADC->ADCMPR[0] |= (0X01ul << 2); // set condition greater than to interrupt
ADC->ADCMPR[0] |= (0x01ul << 1); // interrupt enable at module level
ADC->ADCMPR[0] |= (0x01ul << 0); // compare function enable
	
NVIC->ISER[0] |= (0x01ul << 29); // enable ADC_INT at system level
NVIC->IP[7] &= ~(0x03ul << 29); // set interrupt priority
}
void ADC_IRQHandler(void){
Buzzer_beep(BUZZER_BEEP_TIME); 
CLK_SysTickDelay(BUZZER_BEEP_DELAY);
ADC->ADSR |= (0x01ul << 1); // write 1 to clear CMPF0
NVIC->ICPR[0] |= (0x01ul << 29); // clear pending interrupt of ADC_INT 
}
void Buzzer_beep(int beep_time){
int i;
